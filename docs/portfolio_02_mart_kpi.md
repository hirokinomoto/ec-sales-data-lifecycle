...
- `ec_sales.orders_stg` の粒度は **1行 = 注文明細（order_id × product_id）**
...
`ec_sales.orders_stg` は注文明細粒度のため、1つの注文に複数商品があると `order_id` が複数行になる。
...
これにより `ec_sales.kpi_monthly` が `ec_sales.orders_stg` から定義どおり再現できている。

### 実行結果サマリ

- `ec_sales.kpi_monthly` は月次（1行=1か月）で **6行** 生成された（対象期間：2025-09〜2026-02）。
- 検算SQL（`02_validate_kpi_monthly.sql`）により、`sales_amount / order_count / customer_count` の差分（diff）が **全月で0** であることを確認した。
- `ec_sales.orders_stg` は注文明細粒度のため、`COUNT(*)`（明細行数）と `COUNT(DISTINCT order_id)`（注文件数）が一致しない月があることを確認した（例：2025-09 は 43行 / 23注文）。

---

## 追加拡張：カテゴリ別KPI（任意）
月次KPIに加えて、カテゴリ別KPI（1行 = 月 × カテゴリ）を作成した。  
これにより、BIでカテゴリ別の売上推移や構成比を確認しやすくなる。

- 出力：`ec_sales.kpi_monthly_by_category`
- 作成SQL：`sql/03_mart/04_create_kpi_monthly_by_category.sql`
- 確認SQL：`sql/03_mart/05_check_kpi_monthly_by_category.sql`
- 検算SQL：`sql/03_mart/06_validate_kpi_monthly_by_category.sql`

### 検算（カテゴリ別KPI）
- 売上（sales_amount）はカテゴリ別に合算しても月次KPIと一致する（diff=0 を確認）。
- 注文件数（order_count）/購入者数（customer_count）は「カテゴリ内DISTINCT」であり、カテゴリ別に合算すると月次全体と一致しない場合がある。  
  （同一注文・同一顧客が複数カテゴリにまたがる可能性があるため）