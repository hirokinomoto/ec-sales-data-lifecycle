...
```mermaid
flowchart LR
  A[data/orders_raw.csv\n(架空データ)] --> B[raw: ec_sales.orders_raw\n取り込み専用・加工しない]
  B --> C[raw品質チェック\n欠損/異常値/未来日/想定外status/重複]
  C --> D[staging: ec_sales.orders_stg\n整形 + is_valid_order固定]
  D --> E[first_purchase_date\n初回購入日(有効注文)]
  E --> F[customer_type\nnew/existing(初回購入月ベース)]
  D --> G[mart (#2)\n月次KPI / カテゴリ別KPI]
  G --> H[BI (#3)\nLooker Studio\n分析できる状態]
```  
...
```mermaid
flowchart TB
  subgraph RAW["#1 raw（取り込み専用）"]
    R1[orders_raw] --> R2[品質チェック集計\n・欠損\n・異常値\n・想定外status\n・未来日\n・重複]
  end

  subgraph STG["#1 staging（整形・定義固定）"]
    S1[空文字→NULL\n完全重複除去] --> S2[is_valid_order付与\nKPI対象を固定]
