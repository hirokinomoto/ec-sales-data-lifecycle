...
EC売上データ（架空）を使って、入社〜1年目レベルのデータ職（データアナリスト／データエンジニア）が関わる **実務の一連の流れ（raw → staging → mart → BI）** を、再現可能な形でまとめたポートフォリオです。  
単に「SQLが書ける／ツールが使える」ことの提示ではなく、求人情報を手がかりに業務を分解し、設計・整形・品質管理・検算・可視化までを通しで実装することで、実務の進め方を整理しました。  
（分析結果の正しさを断言するのではなく、**分析できる状態を作ること**を成果物として残しています）
...
- 実務の流れ（raw → staging → mart → BI）を、再現可能な形で実装・整理する
- 「実務がイメージできない」という課題に対し、**求人に基づく業務情報の調査 → 作業の分解 → ポートフォリオで再現**の順でアプローチする
- SQLによる整形・集計（KPI作成、COUNT DISTINCT、検算）の再現性を示す
- データ品質の考え方（事故検知・監視・定義固定）を設計として残す
- 架空データのため、分析結果の断言ではなく「分析可能な状態」の整備に主眼を置く
